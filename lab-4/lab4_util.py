...
def hamming_distance(
    s1: Union[str, List[int], Tuple[int]], s2: Union[str, List[int], Tuple[int]]
):
    distance = 0
...
def matrixRank(mat):
    ...
    for row in range(m):
        pos = next(
            (index for index, value in enumerate(M[row]) if value != 0), -1
        )  # finds position of first nonzero element (or -1 if all 0s)
        if pos > -1:
            for row2 in range(m):
        ...
        for i, stab_info in enumerate(stabilizers):
            ...
            if stab_coords:
                ...
                ax.scatter(
                    ...
                    marker="o",
                    zorder=3,
                    label=f"Stabilizer {stabilizer_type}{stabilizer_index}",
                )
