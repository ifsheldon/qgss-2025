def grade_ex1a(
    ...
    for asset, prices in assets.items():
        if len(prices) <= min_points:
            print(
                f"❌ Asset {asset} has insufficient data. Requires more than {min_points} data points."
            )
            return
    ...
    if (num_gens + 1) * pop_size > 70:
        print(
            "❌ Too many circuits. Ensure (num_generations + 1) * population_size ≤ 70."
        )
        return
...
def grade_ex1b(dpo_result: dict) -> None:
    ...
    if min_cost < global_min:
        print(
            "❌ The minimum cost is lower than the known global minimum. There may be an issue with your implementation."
        )
        return
    ...
    print(
        f"⚠️ The relative deviation from the global minimum is {relative_deviation:.2f}%. Please review your implementation."
    )
...
def grade_ex2(
    ...
    if (
        not isinstance(previous_session_id, list)
        or len(previous_session_id) != 1
        or not isinstance(previous_session_id[0], str)
    ):
    ...
    actual_generations = optimizer_settings.get("de_optimizer_settings", {}).get(
        "num_generations"
    )
    if actual_generations != expected_generations:
        print(
            f"❌ Incorrect number of generations. Expected: {expected_generations}, got: {actual_generations}."
        )
        return
