...
def kicked_ising_1D(
    num_qubits: int, theta_x: float, theta_zz: float, s: int
) -> QuantumCircuit:
    """
    Parameters:
...
def grade_ex2(pubs_ex2: list, backend_name_ex2: str, options_ex2: dict):
    # check types:
    if (
        not isinstance(pubs_ex2, list)
        or not isinstance(backend_name_ex2, str)
        or not isinstance(options_ex2, dict)
    ):
    ...
    all_z = SparsePauliOp.from_sparse_list([("Z" * 4, range(4), 1)], 4)
    ...
    # test options
    if (
        "default_precision" not in options_ex2
        or "max_execution_time" not in options_ex2
        or len(options_ex2) != 2
    ):
        return "Options should have two entries 'default_precision' and 'max_execution_time'"

    if (
        options_ex2["default_precision"] != 0.1
        or options_ex2["max_execution_time"] != 900
    ):
...
def grade_ex3(options_ex3: dict):
    ...
    # test options
    if (
        "default_precision" not in options_ex3
        or "estimate_time_only" not in options_ex3
        or len(options_ex3) != 2
    ):
        return "Options should have two entries 'default_precision' and 'estimate_time_only'"

    if (
        options_ex3["default_precision"] != 0.02
        or options_ex3["estimate_time_only"] != "analytical"
    ):
        return "Value in options field is wrong. Try again"
...
def grade_ex_4_1(circuit: QuantumCircuit):
    ...
    actual_2q_layers = circuit.depth(
    ...
    expected_barriers = 9
    actual_barriers = sum(
        1 for instr in circuit.data if instr.operation.name == "barrier"
    )
...
def grade_ex_4_2(precision_ex4, pubs_ex4, backend_name_ex4, options_ex4):
    ...
    observable_ex4 = qiskit.quantum_info.SparsePauliOp.from_sparse_list(
        [("Z" * n_qubits_ex4, range(n_qubits_ex4), 1)], n_qubits_ex4
    )  # Global Z measurement
    ...
    if not Operator(circuit).equiv(Operator(circ_ex4)):
        return (
            "Circuit should be circ_ex4 created with kicked_ising_1D. Please try again."
        )
    ...
    if (
        not isinstance(options_ex4["max_execution_time"], (int, float))
        or options_ex4["max_execution_time"] > 480
    ):
