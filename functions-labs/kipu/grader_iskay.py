...
def grade_lab_iskay_ex1c(arguments: dict) -> bool:
    ...
    # 4. options
    expected_options = {"shots": 2_500, "num_iterations": 10, "use_session": True}
...
def grade_lab_iskay_ex1d(result: dict) -> bool:
    ...
    if obtained_cost is None:
        ...
        print(
            f"Obtained cost {obtained_cost} does not match expected optimal cost {optimal_cost}. But it is not a failure. Try another backend or increase the number of shots."
        )
        return print_results(True)
    elif obtained_cost == optimal_cost:
        print(
            f"Obtained cost {obtained_cost} matches expected optimal cost {optimal_cost}. "
        )
        return print_results(True)
...
def grade_lab_iskay_ex2b(
    one_local_terms: int, two_local_terms: int, three_local_terms: int
) -> bool:
    """ """
    if (
        not isinstance(one_local_terms, int)
        and not isinstance(two_local_terms, int)
        and not isinstance(three_local_terms, int)
    ):
        return print_results(False)
...
def grade_lab_iskay_ex2c(arguments: dict) -> bool:
    ...
    # 4. options
    expected_options = {"shots": 10000, "num_iterations": 10, "use_session": True}
    if arguments["options"] != expected_options:
        print("Options do not match expected.")
        return print_results(False)
